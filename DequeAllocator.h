...
//
// It uses memory::NodeMemoryResource for the fixed size
...
// At the top of main():
//
//   memory::MemoryPagePool mpp(0x8000);                                 // Allocate 32kB at a time (or larger powers of two are fine too).
//   memory::DequeMemoryResource::Initialization dmrs(&mpp);
//
// Creating the allocator:
//
//   memory::NodeMemoryResource nmr(mpp);                                // For the fixed size element allocation.
//   memory::DequeAllocator<Foo> allocator(nmr);
//
...
// Creating the allocator:
//
//   memory::NodeMemoryResource nmr(AIMemoryPagePool::instance());      // For the fixed size element allocation.
//   memory::DequeAllocator<Foo> allocator(nmr);
//
...
//
//   memory::NodeMemoryResource nmr(mpp, sizeof(Foo38));
//   memory::DequeAllocator<Foo37> allocator37(nmr);
//   memory::DequeAllocator<Foo38> allocator38(nmr);
//   etc.
//
